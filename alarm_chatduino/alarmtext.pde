...
//determine whether to send a schedule test text msg
int sched = 0;
...
char screenName[] = "homealertsys";
// replace with your password
char pass[] = "6912cabernet";

// authorized user.  this user is authorized to send signoff, pinWrite commands
char authUser[] = "monkeypawx02";

//phone number to text
char phonenum[] = "+15107087714";
...
char schedtxt[] = "hello, schedule test text msg";
long schedtime=0;
...
if (alert) {
  ...
  if (client.connected()) {
    if (alertmsg && alarmcount ==0) {
       sendMessage(phonenum, "ALERT ALARM HAS BEEN SET OFF!"); 
       alarmcount ++;
    }
    if (client.available() > 4) {
      ...
      if (readMessage(from, fromLen, msg, msgLen) == 0) {
        ...
        // default reply
        char hello[] = "hi from home alarm system";
        char unknown[] = "unknown command";
        char schedule[] = "test schedule set";
        char notauth[] = "you are not authorized to run this command";        
        char help[] = "command list:\ntest - send message to aim\ntxttest - send message to text\n";
        char help1[] = "ledon - turn on led on the arduino\nledoff - turn off led the arduino\n";
        char help2[] = "schedon - turn on scheduled test txt msg\nschedoff - turn off scheduled test txt msg\n";
        char help3[] = "schedset - set the schedule test txt msg now\nschedstatus - display the hours left for the next sched test msg \n";
        char help4[] = "reconnect - log off and on AIM\nsignoff - sign off AIM\nhelp - display list of commands\nreset - reset the alarm status\n";        
        //strcat(help,"command list:\n");
        //strcat(help,"test - send message to aim\n");
        //strcat(help,"txttest - send message to text\n");
...
//        }
        ...
        } else if (strcasecmp(msg, "help") == 0) {
          sendMessage(from, help);
          sendMessage(from, help1);
          sendMessage(from, help2);
          sendMessage(from, help3);
          sendMessage(from, help4);
        } else if (strcasecmp(msg, "ledon") == 0) {
          alert = 1;
          sendMessage(from, "led on");
        } else if (strcasecmp(msg, "ledoff") == 0) {
          alert = 0;
          sendMessage(from, "led off");
        } else if (strcasecmp(msg, "reset") == 0) {
          alert = 0;
          alertmsg = 0;
          alarmcount = 0;
          sendMessage(from, "alarm status reset");
        } else if (strcasecmp(msg, "schedon") == 0) {
          sched = 1;
          sendMessage(from, "schedule test on");
        } else if (strcasecmp(msg, "schedoff") == 0) {
          sched = 0;
          sendMessage(from, "schedule test off");  
        } else if (strcasecmp(msg, "schedset") == 0) {
          start = millis();
          sendMessage(from, "schedule set at this time. system delay for 15sec");
          delay(1500);
        } else if (strcasecmp(msg, "schedstatus") == 0) {
          if (sched){
             schedtime = (interval - (millis() - start % interval))/3600000;
             itoa(schedtime, msg, 10);
             sendMessage(from, "time left(hours) before next schedule test text:");
             sendMessage(from, msg);
          }
          else{
             sendMessage(from, "schedule test is disabled");
          }         
        } else if (strcasecmp(msg, "reconnect") == 0) {
          client.stop();
          delay(15000);
        } else if (strcasecmp(msg, "test") == 0) {
          sendMessage(from, hello); 
        } else if (strcasecmp(msg, "txttest") == 0) {
           if (strcmp(from, authUser) == 0) {
            sendMessage(phonenum, hello);          
          }
          else{
            sendMessage(from, notauth);
          } 
        } else if (strcasecmp(msg, "signoff") == 0) {
    ...
    //send schedule test text message
    if (sched){
       //it is on time if it is within 1 minute
       if ((millis() - start % interval <= 15000) && (millis() - start >= 0)){
           sendMessage(phonenum, schedtxt);
           delay(15000);
       } 
    }
